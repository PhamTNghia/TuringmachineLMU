...
public class UniversalView implements Initializable {
    @FXML
    ...
    public void enCoding(){
        encodedRules="";
        String[] separate_init_acc_state;
        String[] separate_transaction_function;
        separate_init_acc_state = codeArea.getText().split("000");
        encodedRules+="init: q"+separate_init_acc_state[0].length()+"\n";
        encodedRules+="accept: q"+separate_init_acc_state[1].length()+"\n";
        separate_transaction_function = separate_init_acc_state[2].split("00");
        for (int i = 0; i < separate_transaction_function.length; i++) {
            String[] separate_element = separate_transaction_function[i].split("0");
            encodedRules+="q"+separate_element[0].length()+","+MethodLibrary.translate_Universal_Alphabet(separate_element[1])+"\n";
            encodedRules+="q"+separate_element[2].length()+","+MethodLibrary.translate_Universal_Alphabet(separate_element[3])+","+MethodLibrary.translate_Universal_Direction(separate_element[4])+"\n";
        }
        /*System.out.println(encodedRules);*/
    }

    public void addCodes(ActionEvent event) {
        enCoding();
        lines = encodedRules.split("\n");
